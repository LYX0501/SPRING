...
class UnifyDataset(OFADataset):
    ...
    def process_image_text_pair(self, index):
        ...
        if type == 'simmc2':
            response, info = caption, question

            info_token_list = info.strip().split()
            src_info = ' '.join(info_token_list[-self.max_src_length+2:])

            response_token_list = response.strip().split()
            tgt_response = ' '.join(response_token_list[:self.max_tgt_length-1])

            src_item = self.encode_text(src_info)
            tgt_item = self.encode_text(tgt_response)

        elif type == 'grounded_caption':
            conf = torch.tensor([1.0])
            w, h = image.size
            ...
            region_prefix_item = self.encode_text(f' {question} region:')
            src_item = torch.cat([region_prefix_item, region_coord_item])
            tgt_item = self.encode_text(f' {caption}')
        else:
